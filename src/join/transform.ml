...
and build_context var  proc =
  let rec build_context_aux var =
	...
	[] -> (fun x -> x), (fun x -> x), []
	  ...
	  let news,msgs,pattern = build_context_aux t in
	  ...
	  in 
	  let pattern' = ( cont, [ u ], (swapdir dir) ) :: pattern in
	    news', msgs', pattern' 
  in
    if var = [] then proc else
      let (news, msgs, pattern) = build_context_aux var in
	news (msgs (Trig ((pattern, false), proc)))
	  (* false = non-replicated trigger ???  *)
